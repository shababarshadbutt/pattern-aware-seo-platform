...
Open PRs (don't just push branches) and confirm CI actually runs green in GitHub's environment, not just locally — a passing local run doesn't confirm the CI service containers (e.g. the Postgres service container M1 added) are configured correctly in the real environment. As of M2 this still hasn't happened — four branches (develop, M0, M1, M2) are now unmerged with no PRs opened. Close this out before M3.
Milestone log
