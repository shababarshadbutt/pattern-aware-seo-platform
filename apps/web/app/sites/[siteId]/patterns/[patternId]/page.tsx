...
export default async function PatternDetailPage({
  ...
  const { pattern, latestSample, findings, observations } =
    patternResult.value;
  const siteName =
    ...
    <main className="mx-auto max-w-5xl px-6 py-12">
      ...
      <div className="mt-8 flex divide-x divide-border-subtle border-y border-border-subtle">
        {[
          ...
          <div key={stat.label} className="flex-1 px-4 py-3">
            <div className="font-mono text-2xl leading-none tabular-nums" data-numeric>
              {stat.value}
            </div>
      ...
      {!latestSample ? (
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-8 gap-y-3 text-sm sm:grid-cols-4">
          {[
            ...
            { label: "sample size", value: formatCount(latestSample.sampleSize) },
            {
            ...
            { label: "k requested", value: formatCount(latestSample.kRequested) },
            ...
            <div key={row.label}>
              ...
              <dd className="mt-1 font-mono text-xs tabular-nums text-primary" data-numeric>
                {row.value}
              </dd>
            </div>
          ))}
      ...
      {findings.length === 0 ? (
        ...
        <table className="mt-4 w-full border-collapse text-sm">
          <caption className="sr-only">Published findings for this pattern</caption>
          <thead>
            <tr className="border-b border-border-strong text-left">
              <th scope="col" className="py-2 pr-4 pl-3 font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                Severity
              </th>
              <th scope="col" className="py-2 pr-4 text-right font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                HTTP
              </th>
              <th scope="col" className="py-2 pr-4 text-right font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                Estimate
              ...
              <th scope="col" className="py-2 text-right font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                Computed
              </th>
            </tr>
          </thead>
              ...
              return (
                ...
                >
                  <td className="py-3 pr-4 pl-3">
                    <StatusBadge tone={findingTone} label={finding.severityClass} />
                  </td>
                  <td className="py-3 pr-4 text-right font-mono text-xs tabular-nums text-secondary" data-numeric>
                    {finding.httpStatus}
                  </td>
                  ...
                  <td className="py-3 text-right font-mono text-xs tabular-nums text-secondary" data-numeric>
          ...
          <table className="w-full min-w-[640px] border-collapse text-sm">
            ...
            <thead>
              <tr className="border-b border-border-strong text-left">
                ...
                <th scope="col" className="py-2 pr-4 font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                  Method
                </th>
                <th scope="col" className="py-2 pr-4 font-mono text-2xs font-medium uppercase tracking-wider text-tertiary">
                  Flags
            ...
            <tbody>
              {observations.map((observation) => (
                <tr key={observation.id} className="border-b border-border-subtle">
                  <td className="max-w-[320px] truncate py-3 pr-4 pl-3 font-mono text-xs text-secondary" title={observation.url}>
                    {observation.url}
                  </td>
                  <td className="py-3 pr-4 text-right font-mono text-xs tabular-nums text-secondary" data-numeric>
