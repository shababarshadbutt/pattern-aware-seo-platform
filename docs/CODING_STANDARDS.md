...
A technically-successful run can still be a silent failure. "The job completed without throwing" and "the job did what it was supposed to" are not the same claim, and code should not conflate them. Two concrete instances found in this codebase (M2): a streaming XML parser processing a whole chunk synchronously meant an oversize-file guard never actually fired, and an HTML error page is well-formed enough XML that it parses successfully and yields zero URLs — indistinguishable, downstream, from a site that legitimately lists nothing. Whenever a run can complete "successfully" with a semantically empty or suspicious result (zero items parsed from an accepted input, a count of exactly zero where the historical baseline is never zero), surface that explicitly — a warn-level log at minimum, a dedicated status on the resulting row where one exists (e.g. audit_snapshot/sampling_health) — rather than letting it look identical to an ordinary success. Don't rely on "it didn't throw" as evidence that it worked.
...
Testing: Vitest, plus large adversarial synthetic-corpus tests for tree/streaming algorithms (§1.10).
...
This document should be revised the same way the architecture plan is — when a real decision changes (e.g. a new standards exception like §2.1's partitioned-table primary keys, or the team settles on plural table names instead of singular), update this file rather than letting practice silently drift away from what's written here. As of M2, four such exceptions/additions are recorded here (§1.11 lint tooling, §2.1 partitioned-table primary keys, §1.12 the legacy pattern-extraction algorithm replacement, §2.2 the hash-as-uniqueness-key fix) — see docs/decisions.md for the full ADRs.