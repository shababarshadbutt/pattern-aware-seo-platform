...
Open question from M1, still not resolved by M2: the schema doesn't yet visibly carry forward the shape/stratum distinction (structureClusters/shapeStrata/pattern_shape_rules) that the original review flagged as valuable and that the "Open decisions" section below asks about. M2 addressed pattern-extraction correctness, not this schema question. Needs a direct answer before this doc's PopulationProfile open decision gets marked resolved.
Known operational gap, unresolved as of M2: three branches (develop, the M0 branch, phase-1/m1-schema-tenancy) were unmerged as of M1 with no PRs opened and CI never observed running in GitHub's actual environment (local gh unauthenticated); the M2 report doesn't mention this having been addressed, so treat it as still open, now with a fourth branch (phase-2/m2-sitemap-ingestion) added to the pile. "All gates pass" continues to be a local-only claim. Authenticate gh, push, open the PRs, and confirm a real green CI run before compounding further milestones on an unverified pipeline — this has been flagged after both M1 and M2 now and shouldn't be deferred indefinitely.
...
Status: schema/tenancy foundation complete (M1); single-pass streaming ingestion, pattern extraction (with a corrected, non-legacy algorithm — ADR-0012), and the bounded min-heap sampler complete and benchmarked at 10M-URL scale (M2). The confidence-math and adaptive-expansion bullets (Wilson score interval, finite-population correction, expansion wiring) remain M3's scope — see Implementation progress above.
...
Status: database-level tenant isolation (partitioning by site_id, compile-time query scoping) landed early, in M1, ahead of this phase's original sequencing — the queue/Redis namespacing and connection-pool budgeting pieces of this phase are still pending as of M2.
